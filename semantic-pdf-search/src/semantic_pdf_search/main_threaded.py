...
class Searcher:
    ...
    @staticmethod
    def __read_pages(page_list : list[pymupdf.Page], out_list : list[str]): #, out_progress_index: int,out_progress:list[float]):
        for i,page in enumerate(page_list):
            out_list.append(page.get_text())
            # out_progress[out_progress_index]= i/len(page_list)
            

    @staticmethod
    def forPDF(model : AbstractModel, path : str, out_progress_list:IntVar) -> "Searcher":
        print("before open")
        ...
        for i in range(0,THREADS):
            ...
            threads.append(Thread(target = Searcher.__read_pages, args = (page_lists[i], out_str_lists[i])))#,i,out_progress_list)))
        for t in threads:
            t.start()
        for i,t in enumerate(threads):
            t.join()
            print(f"thread {i} finished")
            out_progress_list.set(int(100*(i/len(threads))))
        texts : list[str] = []
